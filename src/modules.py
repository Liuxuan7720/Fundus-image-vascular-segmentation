...
def validate_model(model, data_val, criterion, epoch, make_prediction=True, save_folder_name='prediction'):
    ...
    for batch, (images_v, masks_v) in enumerate(data_val):
        with torch.no_grad():
            ...
            image_v =images_v[:, :, :].unsqueeze(0).to(device)
            mask_v = masks_v[:, :, :].squeeze(1).to(device)
            # print(image_v.shape, mask_v.shape)
            output_v = model(image_v)
            output_v=output_v[:,:,4:-4,5:-6]
        ...
        if make_prediction:
            ...
            img_cont_np=output_v.cpu().data.numpy()[0]*255
            cv2.imwrite(desired_path + export_name,img_cont_np)
            # img_cont = Image.fromarray(img_cont_np)
            # img_cont.save(desired_path + export_name)
            acc_val = accuracy_check(masks_v, img_cont_np/255)
...
def test_model(model_path, data_test, epoch, save_folder_name='prediction'):
    ...
    for batch, (images_t) in enumerate(data_test):
        ...
        for index in range(images_t.size()[1]):
            with torch.no_grad():
                image_t = images_t[:, index, :, :].unsqueeze(0).to(device)
                # print(image_v.shape, mask_v.shape)
